...
merge m:1 grappe menage using "${data}/ehcvm_welfare_2b_CIV2021.dta"
keep if region != 1 & milieu == 1 // Pour ne rester que dans autres Urbain
keep if _merge==3
drop _merge
...
//Calculer les facteurs de conversion des différents produits
	preserve
		...
		save "${temp}/ehcvm_nsu_abj_CIV2021.dta", replace /* niveau national */
	restore
merge m:1 codpr unite taille using "${temp}/ehcvm_nsu_abj_CIV2021.dta"
keep if _merge==3
drop _merge
