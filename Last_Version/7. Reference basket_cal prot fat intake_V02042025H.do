...
gen cal_day_aeq_only_fh = cal_day_bit / aeq_cal_hh // coefficient d'ajustement
drop _merge
*d = .9924144 
*gen y=1
...
egen refbask_unadjq = sum(cons_tot_kg_aeq_daily), by(food_item)
...
egen refbask_unadjq_cal = mean(cal_int), by(food_item)
gen adjustment_coef = 0.9924144
gen refbask_adjq = refbask_unadjq * adjustment_coef

duplicates drop food_item, force
keep food_item
merge 1:m food_item using "${temp}/cal_intake_foot.dta"

keep food_item fd_kcal fd_pro cons_tot_kg_aeq_daily fd_fat median_price refbask_adjq refbask_unadjq adjustment_coef
