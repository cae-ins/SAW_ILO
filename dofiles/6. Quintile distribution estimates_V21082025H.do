...
preserve
collapse (mean)  total_exp_aeq food_exp_aeq hhsize cal_day_aeq ae_coef_hh (count) freq [aweight =hhweight]
*collapse (mean)  total_exp_aeq food_exp_aeq hhsize cal_day_aeq ae_coef_hh (count) freq , by (quintile)
export excel "${outpts}\COUNTRY_ESTIMATES.xlsx", sheet("Table 2_1_raw") cell(C3) firstrow(var) sheetmodify
restore
...
export excel "${outpts}\COUNTRY_ESTIMATES.xlsx", sheet("Table 7_1_raw") cell(C3) firstrow(var) sheetmodify
restore
...
export excel "${outpts}\COUNTRY_ESTIMATES.xlsx", sheet("Table 20_raw") cell(C3) firstrow(var) sheetmodify
